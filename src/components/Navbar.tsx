...
const Navbar = () => {
  ...
  // Create a ref for the music section
  ...
  const handleLinkClick = (id: string) => {
    ...
    // Scroll to the music section if "Music" is clicked
    if (id === 'music' && musicSectionRef.current) {
      musicSectionRef.current.scrollIntoView({ behavior: 'smooth' });
    }
  };

  return (
    <nav className={`fixed w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-black/90 backdrop-blur-lg' : 'bg-transparent'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          {/* Logo with Link to Home */}
          <div className="flex items-center">
          ...
          <div className="hidden md:block">
            <div className="flex items-center space-x-8">
              {/* Photos and Videos are still using Link for page navigation */}
              {[{ id: 'photos', icon: Image, label: 'Photos', to: '/photos' },
              { id: 'videos', icon: Video, label: 'Videos', to: '/videos' }]
                .map(({ id, icon: Icon, label, to }) => (
                  <Link
              ...
              {/* Music is a button that will scroll to the music section */}
              <button
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden bg-black/95 backdrop-blur-lg">
          <div className="px-2 pt-2 pb-3 space-y-1">
            ...
            {/* Music button for scrolling in mobile view */}
            <button
              onClick={() => handleLinkClick('music')}
              className={`flex items-center space-x-2 w-full px-4 py-3 rounded-lg transition-all duration-300
                ${activeSection === 'music'
